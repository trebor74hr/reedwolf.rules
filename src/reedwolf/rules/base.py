...
class ComponentBase(SetOwnerMixin):
    ...
    def dump_pydantic_models(self, class_dump_store:Optional[List[DumpPydanticClassLines]]=None, path:List[str]=None, depth:int=0, fname:Optional[str]=None):
        # TODO: remove dep somehow - interface?
        from .components import Field, Section, ChoiceField
        from .containers import Extension, Rules


        if class_dump_store is None:
        ...
        for component in children:
            component.dump_pydantic_models(class_dump_store=class_dump_store, path=path[:], depth=depth+1)
        ...
        if fname is not None:
            ...
            with open(fname, "w") as fout:
                fout.write("\n".join(all_lines))
                print(f"Output in {fname}, {len(all_lines)} lines.")
